...
def login_doctor(request):
    return render(request, 'doctor/login.html', {'style': style('login-doctor-style')})
#------------------------------------------------Personal cabinet views-------------------------------------------------
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if post_data['oms'][0] == patient.oms:
                ...
                return render(request, 'patient/card.html', {'style': style('main-style'),
                                                            'meds': med_price_link_name,
                                                            'future_appointments': future_appointments,
                                                            'patient': patient,
                                                            'last_appointments': last_appointments,
                                                            'len_future_appointments': len(future_appointments)}
                                                            )
            else:
                validation = 'Неверный ОМС'
        except ObjectDoesNotExist:
            validation = 'Неверное ФИО'
    return render(request, 'patient/login.html', {'style': style('login-patient-style'), 'validation': validation})

def last_appointment(request, pk):
    last_appointment = MedicalHistory.objects.get(pk=pk)
    return render(request, 'patient/last_appointment.html', {'style': style('main-style'),
                                                            'last_appointment': last_appointment}
                                                            )
#------------------------------------------------End of personal cabinet views-------------------------------------------------
#------------------------------------------------Reception views-------------------------------------------------
def reception_patient(request):
    if request.method == 'POST':
        ...
        try:
            ...
            patient = Patient.objects.create(full_name=post_data['full_name'],
                                            inn=post_data['inn'],
                                            oms=post_data['oms'],
                                            home_adress=post_data['home_adress'],
...
def reception_doctor(request, patient_pk):
    ...
    return render(request, 'reception/doctor.html', {'style': style('main-style'),
...
def reception_datetime(request, pk):
    ...
    return render(request, 'reception/datetime.html', {'style': style('main-style'),
                                                    'weekdays': WEEKDAYS,
